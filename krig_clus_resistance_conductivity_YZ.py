# -*- coding: utf-8 -*-
"""
Created on Wed May 21 10:51:16 2025
...
#%% Ordinary Kriging with y, z axes (conductivity 2D y and z)
#%% Input data
x=np.array([0.85,0.85,0.85,1.15,1.15,1.15,1.45,1.45,1.45]) # x-coordinate (not used in Kriging for YZ plot)
...
# V has been renamed to R to signify Resistance
...
OK = OrdinaryKriging(
    y, # Using y as the first coordinate
    z, # Using z as the second coordinate
    R, # Changed V to R (Resistance)
    variogram_model='gaussian',
    verbose=False, # Changed to False for consistency
    enable_plotting=False,
)

gridy=np.arange(0.0,1.0,0.01,dtype='float64') # Grid for y-coordinate
gridz=np.arange(0.0,0.50,0.005, dtype='float64') # Grid for z-coordinate
zstar,ss=OK.execute("grid", gridy, gridz)


#%% Kriging Plot (y and z)
fig, ax = plt.subplots()
cax = ax.imshow(zstar, extent=(0, 1, 0, 0.5), origin='lower', aspect='equal', cmap='viridis') # Added cmap and aspect
ax.scatter(y, z, marker='.', s=40, facecolors='none', edgecolors='k') # Changed marker size to 40
...
ax.set_yticks(np.arange(0, 0.51, 0.10)) # Adjusted y-ticks

plt.tight_layout()
# Save the plot
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS1.png', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS1.pdf', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS2.png', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS2.pdf', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS3.png', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS3.pdf', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS4.png', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS4.pdf', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS5.png', dpi=400)
# plt.savefig('plots/YZ/spatial_krigtest_2Dyz_TS5.pdf', dpi=400)
plt.close()

#%% Prepare data for clustering
all_y = np.tile(gridy, gridz.shape[0]) # Use gridy and gridz for coordinates
all_z = np.repeat(gridz, gridy.shape[0])
all_res = zstar.flatten() # Renamed all_v to all_res
df = pd.DataFrame({"y_cord": all_y, "z_cord": all_z, "resistance": all_res}) # Columns renamed to 'resistance'
...
k = 3
model = cluster.KMeans(n_clusters=k, init='k-means++', n_init='auto', random_state=0) # Added n_init and random_state
...
fig, ax = plt.subplots()
ax.scatter(df["y_cord"], df["z_cord"], c=df["Color"], s=1) # Using y_cord and z_cord
...
# plt.savefig('plots/YZ/clustering_2Dyz_TS1.png',dpi=400)
# plt.savefig('plots/YZ/clustering_2Dyz_TS1.pdf',dpi=50)
# plt.savefig('plots/YZ/clustering_2Dyz_TS2.png',dpi=400)
# plt.savefig('plots/YZ/clustering_2Dyz_TS2.pdf',dpi=50)
# plt.savefig('plots/YZ/clustering_2Dyz_TS3.png',dpi=400)
# plt.savefig('plots/YZ/clustering_2Dyz_TS3.pdf',dpi=50)
# plt.savefig('plots/YZ/clustering_2Dyz_TS4.png',dpi=400)
# plt.savefig('plots/YZ/clustering_2Dyz_TS4.pdf',dpi=50)
plt.savefig('plots/YZ/clustering_2Dyz_TS5.png', dpi=400)
plt.savefig('plots/YZ/clustering_2Dyz_TS5.pdf', dpi=50)
plt.close()